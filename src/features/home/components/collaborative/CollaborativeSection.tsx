import DedicatedIcon from '#root/assets/images/DedicatedTeamIcon.png'
import ProjectIcon from '#root/assets/images/ProjectsIcon.png'
import TalentMatchingIcon from '#root/assets/images/TalentMachineIcon.png'
import { SectionWrapper } from '#root/components/SectionWrapper'
import {
  CollaborativeItem,
  CollaborativeItemType,
} from '#root/features/home/components/collaborative/CollaborativeItem'
...
export const CollaborativeSection = () => {
  ...
  return (
    <SectionWrapper>
      <Divider sx={{ marginBlock: 15 }} />
      <Typography fontWeight={700} fontSize="2rem" color="#181E54">
        Tailored Collaboration for Maximum Impact
      </Typography>
      <Box component="div" className="ml-auto mr-auto mt-4">
        <Typography fontWeight={400} fontSize="1.1rem" className="text-center">
          Our Pure Play team extension approach allows us to provide you with three distinct collaboration models, each
          designed to best fit your business objectives.
        </Typography>
      </Box>
      <Grid container sx={{ marginTop: 4 }} spacing={16}>
        {collaborativeModels.map((item, index) => (
          <CollaborativeItem data={item} key={item.label} activeIndex={activeIndex} index={index} />
        ))}
      </Grid>
    </SectionWrapper>
  )
